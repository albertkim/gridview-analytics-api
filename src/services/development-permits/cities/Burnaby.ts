  ...
  for (const urlObject of developmentPermitUrls) {
    ...
    for (const parsed of parsedArray) {
      ...
      const response = await AIGetPartialRecords(parsed, {
        instructions: 'The following text extracted from a PDF is messy but do your best. Identify ONLY the items that refer to the development of new structures that are bigger than a single-family residential, laneways, or duplexes. Do not include alterations nor demolitions. Number of units is usually a number listed right after the $ value',
